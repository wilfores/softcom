...
if (!$error) 
    {
	  $int = "INSERT INTO registroarchivo(registroarchivo_hoja_interna,registroarchivo_usuario_inicia, registroarchivo_referencia, registroarchivo_fecha,registroarchivo_fecha_recepcion, registroarchivo_hora_recepcion, registroarchivo_tipo) 
				VALUES ('$_POST[numcite]','$cod_usr','$_POST[tema]','$_POST[fecha]','$_POST[fecha_recepcion]','$_POST[hora_recepcion]','MEMORANDUMS')";
	   ...
	   $temporal=$temp["usuario_memorandum"];
	   $temporal = $temporal + 1;

	   mysql_query("UPDATE usuario SET usuario_memorandum='$temporal' WHERE '$cod_usr'=usuario_cod_usr",$conn);
   
	   $numActualz = mysql_query("SELECT * FROM departamento 
								  WHERE '$cod_depar'=departamento_cod_departamento",$conn);

	   $tempz = mysql_fetch_array($numActualz);
	   $temporalz=$tempz["departamento_memorandum"];
	   $temporalz = $temporalz + 1;
	   mysql_query("UPDATE departamento SET departamento_memorandum='$temporalz' 
					WHERE '$cod_depar'=departamento_cod_departamento",$conn);
		...
		foreach($_POST['tarde'] as $value)
			{
				...
				$int_rca = "INSERT INTO derivaciones(derivaciones_hoja_interna,derivaciones_cod_usr,derivaciones_estado,derivaciones_estadoinicial) VALUES ('$_POST[numcite]','$ocupa','P','P')";
		...
		foreach($tarde2 as $_POST['value2'])
		{
				...
				//echo "$value<br>";//muestra el nombre de usuario
				$cont2++;
		}
...
$correlativo=$rcon["departamento_memorandum"]+1;
$numcite=strtoupper($rcon["departamento_sigla_dep"])."/MEM"."/".$correlativo.$numD;
