...
if($fila_archivo = mysql_fetch_array($sql_valor))
{
  ...
  if($fila_archivo["registroarchivo_estado"] == 'D')
    {
        $valor_submit = "Aprobar";
    }
    else
    {
        $valor_submit = "Recepcionar";
    }
?>
<center>
    <?php
        ...
        <?php
          if($valor_submit=="Aprobar")
          {
                ...
                if(mysql_num_rows($sql_estado) > O)
                {
                     ...
                     echo "<td><center><b><span class=fuente_titulo>Usuario</b></span></center></td>
                           <td><center><b><span class=fuente_titulo>Fecha</b></span></center></td>
...
if(isset($_POST[aprobar]))
{
        ...
        $var_sql = mysql_query("SELECT * FROM registroarchivo
                                WHERE registroarchivo_codigo='$valor_recibido'
                                AND registroarchivo_estado = 'D'",$conn);
        if(mysql_num_rows($var_sql) > 0)
        {
                 mysql_query("UPDATE derivaciones SET
                              derivaciones_situacion='PENDIENTE'
                              WHERE derivaciones_hoja_interna='$valor_recibido'
                              AND derivaciones_estado='P'
                              AND derivaciones_cod_usr='$codigo_usuario'",$conn);
        ...
        else
        {
                 mysql_query("UPDATE derivaciones SET
                              derivaciones_situacion='PENDIENTE',
                              derivaciones_fecha_recepcion='$fecha_actual'
                              WHERE derivaciones_hoja_interna='$valor_recibido'
                              AND derivaciones_estado='P'
                              AND derivaciones_cod_usr='$codigo_usuario'",$conn);
        }
...
<form method="POST">
    <br /><br />
    <input type="submit" name="aprobar" value="<?php echo $valor_submit;?>" class="boton" />
