...
if($_GET['action'] == 'listar')
{
	...
	ORDER BY registrodoc1_fecha_recepcion DESC LIMIT 0,1000
	";
	...
	$query342 = "select usuario_ocupacion, usuario_nombre from usuario where usuario_cod_departamento = '$depto' ";

	$listemp = mysql_query($query342,$conn);


	while($respemp=mysql_fetch_array($listemp)){
		$sqlder = "SELECT derivardoc_cod, derivardoc_hoja_ruta, derivardoc_fec_recibida,
		derivardoc_de, derivardoc_para, derivardoc_proveido,  derivardoc_estado,
		derivardoc_cc, derivardoc_copia_de,
		...
		and derivardoc_gestion = '$anio' limit 0, 1000 ";
		...
		while ($row_der = mysql_fetch_array($rs_sqlder))
		{
			...
			if($row_der["derivardoc_copia_de"] == '0'){
				$rescite = mysql_query("select registrodoc1_cite, registrodoc1_doc, registrodoc1_tipo
						from cites where registrodoc1_hoja_ruta = '$row_der[derivardoc_hoja_ruta]'",$conn);
			}
			else {
				//				echo "es copia*****";
				$rescite = mysql_query("select registrodoc1_cite, registrodoc1_doc, registrodoc1_tipo
						from cites where registrodoc1_hoja_ruta = '$row_der[derivardoc_copia_de]'",$conn);
			}
