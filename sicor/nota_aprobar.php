...
if(isset($_POST[aprobar]))
{
    ...
    if($error == 1)
    {
        ...
        mysql_query("UPDATE registroarchivo SET
                     registroarchivo_estado = 'D'
                     WHERE registroarchivo_codigo='$valor_recibido'",$conn);
					 
		// Cambiamos de estado Devivaciones			 
		mysql_query("UPDATE derivaciones SET
                     derivaciones_proveido = 'N'
                     WHERE derivaciones_cod_usr='$codigo_usuario' AND derivaciones_hoja_interna='$valor_recibido'",$conn);
					 			 
        $datosvar=mysql_query("SELECT derivaciones_proveido FROM derivaciones
                     WHERE derivaciones_hoja_interna='$valor_recibido' and derivaciones_estado='V'",$conn);								 
		$ctrol=0;

		while($datosarchivo = mysql_fetch_array($datosvar))
            {  $dat1=$datosarchivo["derivaciones_proveido"];
                if($dat1 == 'S')
				 { $ctrol=1;
				 }
				 
				 echo "DATOS1 = $datosarchivo[derivaciones_proveido]<br>";
				 
		    } 
		if($ctrol!=1)
		   { mysql_query("UPDATE derivaciones SET
                     derivaciones_proveido = 'N'
                     WHERE derivaciones_hoja_interna='$valor_recibido' AND derivaciones_estado='P'",$conn);	
		   }
        ?>
...
<form method="POST">
    <br /><br />
    <input type="submit" name="aprobar" value="Aprobar" class="boton" />
