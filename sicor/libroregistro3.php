...
<center><b style=" font-size:18px">LISTADO DE RECIBIDOS</b></center></p>
<center>
<form name="formfiltro" action="libroregistro3.php" method="POST">
	Hoja de Ruta: <input type="text" name="h_rut" value="<? echo $h_rut; ?>" size=10>
    &nbsp; &nbsp;
	Nro de cite: <input type="text" name="n_cit" value="<? echo $n_cit; ?>" size=10>
    &nbsp; &nbsp;
	Tipo de Documento: <select name="t_doc" style="width:200px; font-size:9px ">
    ...
    &nbsp; &nbsp;  
    <input type="button"  value="Ver Todo" class="boton" onClick="window.location='libroregistro3.php'">
</form>
<!--  style="font-size:9px; color:blue;"-->
</center>
<table width="0" border="0" cellpadding="0" align="center">
  <tr>
    <td class="nuevos">CORRESPONDENCIA NUEVA</td>
    <td class="externos">CORRESPONDENCIA EXTERNA</td>
    <td class="derivados">CORRESPONDENCIA DERIVADA</td>
  </tr>
</table>
...
<tr style="font-size: 8pt; color:#FFFFFF" bgcolor="#3E6BA3">
	...
	<td width="8%" align="center"><span >Fecha Recepcion</td>
...
$slista="SELECT 
registrodoc1_id, registrodoc1_hoja_ruta, registrodoc1_fecha_recepcion, registrodoc1_de, usuario_nombre, registrodoc1_referencia, documentos_descripcion, registrodoc1_estado, registrodoc1_asociar, registrodoc1_asociar_h_r, registrodoc1_cc, registrodoc1_cite, usuario_ocupacion, registrodoc1_asociar, documentos_id
...
//**********************--------------------------------------------------------------USUARIO EXTERNO
$slista2="SELECT registrodoc1_id, registrodoc1_hoja_ruta, 
		registrodoc1_fecha_recepcion, registrodoc1_de, 
...
if($h_rut!=""){
$slista2 = $slista2. "and registrodoc1_hoja_ruta like '%$h_rut%' ";
}
if($t_doc!=""){
$slista2 = $slista2. "and documentos_descripcion like '%$t_doc%' ";
}
if($n_cit!=""){
$slista2 = $slista2. "and registrodoc1_cite like '%/$n_cit/%' ";
}
//$slista = $slista. "ORDER BY documentos_descripcion desc, registrodoc1_cite";
$slista2 = $slista2. "ORDER BY registrodoc1_fecha_recepcion DESC";
$rslista2=mysql_query($slista2,$conn);

 while($rwlista2=mysql_fetch_array($rslista2))
 {
	 //echo "venga!";
	 $doc_arch=cifrar($rwlista2["registrodoc1_hoja_ruta"]);
	 $doc_arch2=cifrar($rwlista2["registrodoc1_cite"]);
	 $doc_aso=$rwlista2["registrodoc1_asociar"];
 	
		
	 $doc_rem = $rwlista2["registrodoc1_de"];
     if ($resaltador==0)
	  {
//       echo "<tr class=truno>";
...
//       echo "<tr class=trdos>";
       echo "<tr class=externos>";
		...
		// obtenemos el nombre del remitente del correo
		$econs = "select usuarioexterno_nombre from usuarioexterno where usuarioexterno_codigo= '$doc_rem'";
		$eresp = mysql_query($econs, $conn);
		$remitente = mysql_fetch_array($eresp);		
		//******
...
?>
	<td align="center">
    	<input type="checkbox" name="<?php echo "check".$indice; ?>" value="<?php echo $rwlista2["registrodoc1_id"];?>" onclick="armar(this.value);">
    
    </td>
    <td align="center"><?php 
    if($rwlista2["registrodoc1_asociar"]=='si'){echo "<font color=#CC0000>Doc. Asociado $rwlista[registrodoc1_asociar_h_r]";}
    else {echo $rwlista2["registrodoc1_hoja_ruta"];}?>
    </td>
    
    <td align="center"><?php echo $rwlista2["registrodoc1_cite"];?>
    </td>
    
    <td align="center"><?php echo $rwlista2["registrodoc1_fecha_elaboracion"];?>
    </td>

    <td align="left"><?php echo $remitente["usuarioexterno_nombre"];?></td>
    
    <td align="left"><?php echo $rwlista2["usuarioexterno_nombre"];?>
    </td>
    
    
    <td align="left"><?php echo $rwlista["descri"];?></td>
    
    <?php
    if($rwlista2['registrodoc1_estado']=='SR') 
    {$rep='No Recepcionado';?><td align="center" style="color:#990000; font-size:10px"><?php echo "$rep";?></td>
    <?php }
    else {$rep='Recepcionado';?>
    <td align="center" style="font-size:10px"><?php echo "$rep";?></td>
...
//**********************---------------------------------------------------fin muestra del usuario externo
...
<form name="formimprime" action="imprimirlibroreg3.php" method="GET" target="_blank">
